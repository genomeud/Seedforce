...
# prepare input table
awk -F'\t' -v OFS='\t' '{ for (i = 1; i <= NF; ++i) sub(/^$/, 0, $i) } 1' ${out2}/populations.fst_summary.tsv | cat - <(echo -e "PLA\t0\t0\t0\t0\t0\t0\t0\t0") > ${out2}/populations.fst_summary_for_heatmap.tsv
...
# # # # # # # # # THIS PART IS NOT DONE YET BECAUSE WE DONT HAVE GEOGRAPHIC LOCATION WHERE SAMPLES WERE COLLECTED!!!!
