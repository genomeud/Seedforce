...
option_list = list(
  ...
  default="linaria_flava",help="spp name [default= %default]", metavar="character"),
  ...
  default="SNPRelate/linaria_flava.cov5.info50.gds",help="SNPRelate gds file [default= %default]", metavar="character"),
  ...
  default="SNPRelate/linaria_flava.cov5.info50.ibd", help="IBD matrix output file [default= %default]", metavar="character"),
...
run_pca<-function(spp,vcffile,popfile)
{
    ...
    total$color_group[total$group_assignment=="RMA"]<-col4
    total$color_group[total$group_assignment=="LSO"]<-col5
    total$color_group[total$group_assignment=="ISS"]<-col6
    total$color_group[total$group_assignment=="PLA"]<-col7
    total$color_group[total$group_assignment=="TON"]<-col8
    total$color_group[total$group_assignment=="RIC"]<-col9
    total$color_group[total$group_assignment=="MOL"]<-col10
    total$color_group[total$group_assignment=="CRE"]<-col11
    
    color_list<-c(col4,col5,col6,col7,col8,col9,col10,col11)
	...
	##########
    par(family="Calibri")
    legend("topright",legend=c(expression(paste(italic("RMA"))),expression(paste(italic("LSO"))),expression(paste(italic("ISS"))),expression(paste(italic("PLA"))),expression(paste(italic("TON"))),expression(paste(italic("RIC"))),expression(paste(italic("MOL"))),expression(paste(italic("CRE")))),text.col=c(c(color_list)[1:8]),ncol=1,x.intersp=0.7,text.font=c(4),cex=cex.legend,pch=c(0),col=c(c(color_list)[1:8]),bty = "n",bg =NA)
    dev.off()
...
run_ibd<-function(gdsfile,to_remove,popfile,within,ibdmatfile,maf,missingness,LD)
{
    ...
    intra$POP1<-factor(intra$POP1,levels=c("RMA","LSO","ISS","PLA","TON","RIC","MOL","CRE","all"))
    color<-c("gray68","orchid2","seagreen","tomato2","hotpink4","steelblue","lightgoldenrod","cyan3","white")
    ...
    #Plot within pop relatedness
    out$POP1<-factor(out$POP1,levels=c("RMA","LSO","ISS","PLA","TON","RIC","MOL","CRE"))
    ...
    text(c(1:8),-0.18,c("RMA","LSO","ISS","PLA","TON","RIC","MOL","CRE"),adj=0.5,padj=0.5,font=4,cex=cex.axis,xpd=NA,las=1)
