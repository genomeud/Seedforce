...
option_list = list(
  ...
  default="galium_litorale",help="spp name [default= %default]", metavar="character"),
  ...
  default="/projects/marroni/seedforce/galium_litorale/raw_reads/sample_ID_and_populations.txt",help="Population file path [default= %default]", metavar="character"),
  make_option(c("-G", "--gdsfile"), type="character",
  default="SNPRelate/galium_litorale.cov5.info50.gds",help="SNPRelate gds file [default= %default]", metavar="character"),
  ...
  default="SNPRelate/galium_litorale.cov5.info50.ibd", help="IBD matrix output file [default= %default]", metavar="character"),
...
colors=c("dimgray","palevioletred1","springgreen4","chocolate","hotpink4","steelblue","chartreuse1","mediumpurple","yellow4","orange","darkseagreen","coral4","black")
# galium
...
# label_positions <- aggregate(cbind(EV1, EV2) ~ group_assignment, data = total, FUN = mean)
	...
	pc1=1
	pc2=2
	plot(test$EV1,test$EV2,col=total$color_group,cex=cex.cross,pch=pch,xlim=c(x_le,x_ri+0.05),ylim=c(y_low,y_up),xlab="",ylab="",font.lab=2,las=1,cex.axis=cex.axis,xaxt="n",yaxt="n",tck=-0.01,main=c(expression(paste(italic("G. litorale")))),cex.main=cex.main)   
    # add the panel letter
    pos=4
    mtext(paste0(toupper(LETTERS[pos]), ")"), 2,line=0.3,at=0,padj=-11.5,adj=2,cex=cex_text,las=2,font=2)
...
run_ibd<-function(gdsfile,to_remove,popfile,within,ibdmatfile,maf,missingness,LD)
{
    ...
    # set colors to pops
    library(viridis)
    colors_list=c("gray68","orchid2","seagreen","tomato2","hotpink4","steelblue","chartreuse1","mediumpurple","yellow4","orange","darkseagreen","coral4","black")
    # galium
    pops=c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1","ALL")
    ...
    intra$POP1<-factor(intra$POP1,levels=c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1","ALL"))
    color<-colors_list
    # ---------------------- #
    ...
    #Plot within pop relatedness
    out$POP1<-factor(out$POP1,levels=c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1","ALL"))
...
# Read vcf file
vcff <- "/projects/marroni/seedforce/galium_litorale/de_novo_pipeline/populations_stacks/populations.snps.filtered.recode_MIS_filt_header.vcf"
invcf <- vcff
vcf <- read.vcfR(invcf)
# extract genotypes
gt <- extract.gt(vcf)
...
write.table(result, "genotype_counts_galium_litorale.tbl", row.names = FALSE, quote=FALSE,sep="\t")
...
png(outfile,width=25,height=7,units="cm",res=res, type="cairo")
...
ordine <- c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1","ALL")
full$Population <- factor(full$Population, levels = ordine)
posi<-stripchart(full$Heterozygous_Ratio~full$Population,method = "jitter",cex.axis=cex.axis,ylab="",xlab="",font=2,col=color, yaxt="n",xaxt="n", vertical=TRUE,ylim=c(-0.02,0.3),cex=cex,pch=pch, main=c(expression(paste(italic("G. litorale")))))
...
# browser()
posi$names <- c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1","ALL")
text(seq(1,length(posi$names),1),par('usr')[3]-((25*par('usr')[3])/100),myletters,cex=cex.significance,font=2,adj=c(0.5,0.5) )

# add pop names
axis(1,at=c(1:13),tick=T,labels=FALSE,adj=0.5,padj=0.5,font=c(4),cex.axis=cex.axis)
text(c(1:12),-0.052,c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(13),-0.052,c("ALL"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(1:12),-0.068,c("8","9","8","9","6","8","6","8","9","9","8","8"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(13),-0.068,c("96"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
...
posi<-stripchart(intra$kinship~intra$POP1,method = "jitter",col=color,cex.axis=cex.axis,ylab="",xlab="",font=2,yaxt="n",xaxt="n",ylim=c(-0.02,0.5), vertical=TRUE,cex=cex,pch=pch,main=c(expression(paste(italic("G. litorale")))))
...
# add pop names
axis(1,at=c(1:13),tick=T,labels=FALSE,adj=0.5,padj=0.5,font=c(4),cex.axis=cex.axis)
text(c(1:12),-0.072,c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(13),-0.072,c("ALL"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(1:12),-0.097,c("8","9","8","9","6","8","6","8","9","9","8","8"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(13),-0.097,c("96"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)

#############################
# PLOT C # inbreeding (MLE) #
#############################
write.table(out, "IBD_values.txt", sep="\t")
posi<-stripchart(out$indinb.coeff.mle~out$POP1,method = "jitter",col=color,cex.axis=cex.axis,ylab="",xlab="",lab.font=2,yaxt="n",xaxt="n",ylim=c(-0.05,1), vertical=TRUE,cex=cex,pch=pch,main=c(expression(paste(italic("G. litorale")))))
axis(2,line=0,las=ylas,cex.axis=cex.axis,mgp=c(0,0.15,0), tck = 0)
mtext("Inbreeding coefficient",2,font=2,line=1.8,cex=cex.lab)
# add pop names
axis(1,at=c(1:13),tick=T,labels=FALSE,adj=0.5,padj=0.5,font=c(4),cex.axis=cex.axis)
text(c(1:12),-0.155,c("SOS","BIA","BCO1","BCO2","BA2","BA5","BA6","BA4","BA1","CUS","3F","TR1"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(13),-0.155,c("ALL"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(1:12),-0.205,c("8","9","8","9","6","8","6","8","9","9","8","8"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
text(c(13),-0.205,c("96"),adj=0.5,font=2,cex=cex.text,xpd=NA,las=1)
