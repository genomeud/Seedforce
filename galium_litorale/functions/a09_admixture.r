...
admixture.plot.no_low_cov.cov5.info50<-function(indir,outdir,file.prefix, k_value="1.Q",popfile,res=1600)
{
    ...
    outfile=paste(outdir,"/",file.prefix,".",k_value,"color_groups.png", sep="")
    ...
    # titolo<-paste("G. litorale (K=", gsub(".Q","",k_value), ")", sep="")
k_clean <- gsub(".Q", "", k_value)
titolo <- bquote(italic("G. litorale") ~ "(K=" * .(k_clean) * ")")


    res=res
    cex.text=0.6
    png(outfile,width=14.5,height=7.5,units="cm",res=res, type="cairo")
    par(mar=c(1.1,2.5,1.4,0), mgp=c(1.6,0.5,0))
    ...
    color=c("dimgray","palevioletred1","springgreen4","chocolate","hotpink4","steelblue","chartreuse1","mediumpurple","yellow4","orange","darkseagreen","coral4")
    

    coord<-barplot(t(as.matrix(tbl_ord)), col=color, main=titolo, ylab="Ancestry", border=NA, cex.axis=0.8, cex.main=0.85, las=2, cex.lab=0.85, cex.names=1, space=0.25,tck=-0.02,font.lab=2)
...
plot_cv_evanno_validation<-function(infile=infile,outdir=outdir,file.prefix=file.prefix,res=1600)
{
    ...
    # draw CV plot 
	outfile=paste(outdir,"/",file.prefix,".evanno_cv_validation_error",".png", sep="")
	png(outfile,res=res,width=10,height=6,units="cm",type="cairo")
